...
<jsp:include page="/header.jsp"/>
    ...
    <form action='/project/update'>
        ...
        기간: <input name='startDate' type='date' value='<%=project.getStartDate()%>'> ~
        ...
        %>
            <li>
                <input <%=isMember(project.getMembers(), user) ? "checked" : ""%>
                    name='member'
                    value='<%=user.getNo()%>'
                    type='checkbox'> <%=user.getName()%>
            </li>
        <%
            }
        %>
        ...
        <button type='button' onclick='location.href="/project/delete?no=<%=project.getNo()%>"'>삭제</button>
    </form>
